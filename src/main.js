...
function addNewActivity() {
  ...
  newActivity.innerHTML = `
    <div class="activity-info" data-activity="${activityCount}">
      ...
      <p class="target-time">Target time: 00:00.00</p>
    </div>
  `;

  document.querySelector(".container__current_activity ul").appendChild(newActivity);

  // Add event listeners for the new start/stop buttons
  newActivity.querySelector(".start").addEventListener("click", (e) => {
    const activityid = e.target.getAttribute("data-activity");
    startTimer(activityid);
  });

  newActivity.querySelector(".stop").addEventListener("click", (e) => {
    const activityid = e.target.getAttribute("data-activity");
    stopTimer(activityid);
  });
}
...
function stopTimer(activityid) {
  if (timers[activityid]) {
    ...
    const targetTimeStr = document
      .querySelector(.activity-info[data-activity="${activityid}"] .target-time)
...
function activities() {
  ...
  savedActivities.forEach(({ activity, target, time, performance }) => {
    const row = document.createElement("tr");
    row.innerHTML = <td>${activity}</td><td>${target}</td><td>${time}</td><td>${performance}</td>;
    document.querySelector("#activityTable tbody").appendChild(row);
  });
